...
String & String::copy(const __FlashStringHelper *pstr, unsigned int length) {
    ...
    setLen(length);
    memcpy(wbuffer(), (PGM_P)pstr, length + 1); // We know wbuffer() cannot ever be in PROGMEM, so memcpy safe here
    return *this;
}
...
unsigned char String::concat(const __FlashStringHelper * str) {
    if (!str) return 0;
    int length = strlen((PGM_P)str);
    if (length == 0) return 1;
    unsigned int newlen = len() + length;
    if (!reserve(newlen)) return 0;
    memcpy(wbuffer() + len(), (PGM_P)str, length + 1);
    setLen(newlen);
    return 1;
}
